function(fac_lpr_import_onnxruntime root)
    ...
    elseif(APPLE)
        set(_ort_library "${_ort_root}/lib/libonnxruntime.dylib")
        if(NOT EXISTS "${_ort_library}")
            file(GLOB _ort_candidates "${_ort_root}/lib/libonnxruntime*.dylib")
            list(SORT _ort_candidates)
            if(_ort_candidates)
                list(GET _ort_candidates 0 _ort_library)
            endif()
        ...
        if(NOT EXISTS "${_ort_library}")
            message(FATAL_ERROR "Invalid macOS ONNX Runtime package under ${_ort_root}")
        endif()
        ...
        message(FATAL_ERROR "Prebuilt ONNX Runtime bootstrap currently supports Windows x64, Linux x64, and macOS ARM64")
    endif()
endfunction()
