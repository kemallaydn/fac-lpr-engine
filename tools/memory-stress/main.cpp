...
struct Options final {
    ...
    std::uint64_t max_loop_growth_bytes{128ULL * 1024ULL * 1024ULL};
    std::uint64_t max_lifecycle_growth_bytes{256ULL * 1024ULL * 1024ULL};
};
...
[[nodiscard]] std::optional<std::size_t> parse_size(std::string_view text) {
    ...
    if (end == nullptr || *end != '\0' || value > std::numeric_limits<std::size_t>::max()) {
        return std::nullopt;
    }
...
[[nodiscard]] std::optional<double> parse_double(std::string_view text) {
    if (text.empty()) return std::nullopt;
    char* end = nullptr;
    const auto value = std::strtod(std::string{text}.c_str(), &end);
    if (end == nullptr || *end != '\0' || value < 0.0) return std::nullopt;
    return value;
}

[[nodiscard]] std::size_t profile_iterations(std::string_view value) {
...
[[nodiscard]] std::optional<Options> parse_options(const int argc, char** argv) {
    ...
    for (int index = 2; index < argc; ++index) {
        ...
        } else if (arg == "--handle-cycles") {
            const auto parsed = parse_size(value);
            if (!parsed) return std::nullopt;
            options.handle_cycles = *parsed;
        } else if (arg == "--max-loop-growth-mib") {
            const auto parsed = parse_double(value);
            if (!parsed) return std::nullopt;
            options.max_loop_growth_bytes = static_cast<std::uint64_t>(*parsed * 1024.0 * 1024.0);
        } else if (arg == "--max-lifecycle-growth-mib") {
            const auto parsed = parse_double(value);
            if (!parsed) return std::nullopt;
            options.max_lifecycle_growth_bytes = static_cast<std::uint64_t>(*parsed * 1024.0 * 1024.0);
        } else {
            return std::nullopt;
        }
    }
...
[[nodiscard]] fac_lpr::application::ImageView image_view(const cv::Mat& image) {
...
[[nodiscard]] std::uint64_t positive_growth(
    const std::uint64_t baseline,
    const std::uint64_t current) noexcept {
    return current > baseline ? current - baseline : 0U;
}

[[nodiscard]] std::uint64_t average_tail_rss(const std::vector<RssSample>& samples) {
    if (samples.empty()) return 0U;
    const auto tail_count = std::max<std::size_t>(1U, samples.size() / 4U);
    const auto start = samples.size() - tail_count;
    long double total = 0.0L;
    for (std::size_t index = start; index < samples.size(); ++index) {
        total += static_cast<long double>(samples[index].bytes);
    }
    return static_cast<std::uint64_t>(total / static_cast<long double>(tail_count));
}

void run_handle_lifecycle(const std::size_t cycles) {
    for (std::size_t index = 0U; index < cycles; ++index) {
        ...
        const auto create_status = fac_lpr_engine_create_v1(&config, &handle);
        if (create_status != FAC_LPR_STATUS_OK || handle == nullptr) {
            throw fac_lpr::application::InternalError("C ABI create failed during lifecycle stress");
        }
        const auto destroy_status = fac_lpr_engine_destroy_v1(&handle);
        if (destroy_status != FAC_LPR_STATUS_OK || handle != nullptr) {
            throw fac_lpr::application::InternalError("C ABI destroy failed during lifecycle stress");
        }
        const auto repeated_destroy = fac_lpr_engine_destroy_v1(&handle);
        if (repeated_destroy != FAC_LPR_STATUS_OK) {
            throw fac_lpr::application::InternalError("C ABI repeated destroy failed during lifecycle stress");
        }
    }
}
...
void write_report(
    ...
    const bool pass) {
    const auto parent = options.report.parent_path();
    if (!parent.empty()) std::filesystem::create_directories(parent);
    std::ofstream output{options.report};
    if (!output) {
        throw fac_lpr::application::InternalError("cannot create memory stress report");
    }
    output << "{\n"
           << "  \"schemaVersion\": 1,\n"
           ...
           << "  \"lifecycleCycles\": " << options.lifecycle_cycles << ",\n"
           ...
           << "  \"loopFinalGrowthBytes\": " << positive_growth(loop_baseline, loop_final) << ",\n"
           << "  \"loopTailGrowthBytes\": " << positive_growth(loop_baseline, loop_tail_average) << ",\n"
           << "  \"handleLifecycleGrowthBytes\": " << positive_growth(handle_before, handle_after) << ",\n"
           << "  \"pipelineLifecycleGrowthBytes\": " << positive_growth(lifecycle_before, lifecycle_after) << ",\n"
...
int main(const int argc, char** argv) {
    ...
    try {
        ...
        const auto loop_tail_average = average_tail_rss(samples);
        pipeline.reset();

        const auto handle_before = current_rss_bytes();
        run_handle_lifecycle(options->handle_cycles);
        ...
        const auto loop_final_growth = positive_growth(loop_baseline, loop_final);
        const auto loop_tail_growth = positive_growth(loop_baseline, loop_tail_average);
        const auto handle_growth = positive_growth(handle_before, handle_after);
        const auto lifecycle_growth = positive_growth(lifecycle_before, lifecycle_after);
        const bool pass =
        ...
        std::cout << "FAC LPR memory stress: iterations=" << options->iterations
                  << " loopGrowth=" << loop_final_growth
                  << " tailGrowth=" << loop_tail_growth
                  << " handleGrowth=" << handle_growth
                  << " lifecycleGrowth=" << lifecycle_growth
